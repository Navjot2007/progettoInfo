...
try {
    ...
    if (isset($_POST['azione'])) {
        ...
        if (
            ...
            empty($_POST['luogo']) ||
            empty($_POST['data']) ||
            empty($_POST['costoTotale']) ||
            !is_numeric($_POST['costoTotale'])
        ) {
            die("Inserisci i dati correttamente!");
        }
        ...
        $data = $_POST['data'];
        $nickname = 'user1';
        $costoTotale = $_POST['costoTotale'];


        $stmt = $pdo->prepare("
            INSERT INTO evento (titolo, luogo, `data`, nickname, costoTotale)
            VALUES (:titolo, :luogo, :data, :nickname, :costoTotale)
        ");
        $stmt->bindParam(':titolo',$titolo);
        $stmt->bindParam(':luogo',$luogo);
        $stmt->bindParam(':data',$data);
        $stmt->bindParam(':nickname',$nickname);
        $stmt->bindParam(':costoTotale',$costoTotale);
        ...
        if (!isset($data)) $data = "";
        if (!isset($costoTotale)) $costoTotale = "";
    }
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
    <table>
        ...
        <tr>
            <td>Costo totale:</td>
            <td><input type="text" name="costoTotale"></td>
        </tr>
    </table>
